...
Migrate in deletion-producing tranches. Hartmann, Hunt, and the operator check
now follow the linear contract, and the example-only `lmx.example_runner`
surface has been deleted after its reusable snapshot helper moved to the
showcase owner. Next make variable-field, fringing, restart,
