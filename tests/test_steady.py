...
# resolution of 64 points per direction; Ha 1000 from its parity quadrant on 48 mapped points,
# because the full-domain solve at 96 points is 0.60 % high there. Held as constants so a
# solver test costs a solve and not a dense spectral factorization;
...
# Hunt's duct, keyed by (Ha, wall conductance ratio): Ha 20 on 64 points, Ha 300 on the
# mapped quadrant (48 points, beta 5), regenerated by the Hunt test below.
HUNT_FLOW_RATE = {
    (20.0, 0.027): 0.027577890,
    (20.0, 0.100): 0.017148080,
    (300.0, 0.027): 0.00041667,
    (300.0, 0.100): 0.00017073,
}
...
def test_the_reference_values_are_what_the_spectral_solve_returns():
    ...
    for hartmann, cached in REFERENCE_FLOW_RATE.items():
        spectral = quadrant_flow_rate(hartmann) if hartmann > 300.0 else flow_rate(hartmann, 64)
        assert spectral == pytest.approx(cached, rel=2e-4)
...
def test_a_hunt_duct_matches_the_spectral_reference(conductance):
    ...
    exact = HUNT_FLOW_RATE[20.0, conductance]
...
def test_the_hunt_reference_values_are_what_the_spectral_solve_returns():
    for (hartmann, conductance), cached in HUNT_FLOW_RATE.items():
        spectral = (
            quadrant_flow_rate(hartmann, hartmann_wall=conductance)
            if hartmann > 100.0
            else flow_rate(hartmann, 64, hartmann_wall=conductance)
        )
        assert spectral == pytest.approx(cached, rel=2e-4)
