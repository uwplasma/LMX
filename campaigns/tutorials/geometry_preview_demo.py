...
def write_preview_bundle(
    *, out_dir: Path, with_post_run: bool = False, post_case_kind: str = "hartmann"
) -> dict[str, object]:
    ...
    (out_dir / "geometry_preview_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Preview LMX geometries and generate matching postprocessing figures."
    )
    parser.add_argument(
        "--output", type=Path, default=Path("artifacts/examples/geometry_preview")
    )
    parser.add_argument(
        "--with-post-run",
        action="store_true",
    ...
    write_preview_bundle(
        out_dir=args.output,
        with_post_run=args.with_post_run,
        post_case_kind=args.post_case,
    )
    return 0
