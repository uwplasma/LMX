...
JAX_CACHE_DIR = REPOSITORY_ROOT / "artifacts" / "jax_cache"
if str(WORKFLOW_DIR) not in sys.path:
    sys.path.insert(0, str(WORKFLOW_DIR))

from geometry_panel_demo import run_geometry_panel_demo  # noqa: E402


def _hartmann_b_from_ha(
    *,
    ha: float,
    hartmann_spacing: float,
    conductivity: float,
    density: float,
    viscosity: float,
) -> float:
    ...
    *,
    ...
    if include_movie:
        ...
        if movie_case_kind == "hartmann":
            ...
            movie_case = replace(
                ...
                magnetic_field=MagneticFieldSpec(
                    kind="constant", value=(0.0, corrected_bmag, 0.0)
                ),
            )
        ...
        movie_outputs = write_transient_movies(
            ...
            if movie_case_kind in {"hunt", "shercliff", "hartmann"}
            else (),
            view_elev=20.0,
            view_azim=-64.0,
        )
    ...
    (out_dir / "readme_showcase_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate the README showcase media bundle."
    )
    parser.add_argument("--output", type=Path, default=Path("docs/_static/generated"))
    parser.add_argument(
        "--movie-case-kind", choices=("hartmann", "hunt"), default="hunt"
    )
