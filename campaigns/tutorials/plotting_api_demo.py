...
def run_plotting_api_demo(
    *,
    ...
    geometry_paths = write_geometry_preview_plots(
        steady.mesh, out_dir / "geometry", case_title=case.name
    )
    overview_paths = write_case_overview_plots(
        steady, out_dir / "steady", case_title=case.name
    )
    ...
    frames = solve_case_snapshots(
        transient_case, frame_count=max(1, int(movie_t_final / movie_dt))
    )
    ...
    (out_dir / "plotting_api_demo_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Demonstrate the importable LMX plotting APIs."
    )
    parser.add_argument(
        "--output", type=Path, default=Path("artifacts/examples/plotting_api_demo")
    )
