...
def print_solution_log(solution, *, max_rows: int = 14) -> None:
    ...
    print("\n--- Solver progress log ---")
    print(
        "Time        | max|u|      | mean(u)     | max|J|      | max|JxB|    | U-res       | phi-res     | phi-it"
    )
    print("-" * 111)
    for idx in selected:
        print(
...
def save_solution_npz(solution, case, path: Path, *, case_kind: str, ha: float) -> Path:
    ...
    metadata = {
        ...
        "description": "LMX steady solution dump written by campaigns/tutorials/theory_meeting_demo.py",
    }
    diag = solution.diagnostics
...
def save_snapshot_npz(
    ...
    *,
    ...
    np.savez_compressed(
        ...
        potential_residual_history=np.asarray(
            [float(frame["potential_residual"]) for frame in frames]
        ),
        potential_iterations_history=np.asarray(
            [float(frame["potential_iterations"]) for frame in frames]
        ),
        face_current_max_history=np.asarray(
            [float(frame["face_current_max"]) for frame in frames]
        ),
        emf_max_history=np.asarray([float(frame["emf_max"]) for frame in frames]),
        mean_velocity_history=np.asarray(
            [float(frame["mean_velocity"]) for frame in frames]
        ),
        applied_forcing_history=np.asarray(
            [float(frame["applied_forcing"]) for frame in frames]
        ),
        pressure_proxy_history=np.asarray(
            [float(frame["pressure_proxy"]) for frame in frames]
        ),
    )
...
def run_steady_case(
    ...
    *,
    ...
    write_profile_csv(
        case_dir / f"{case.name}_centerline.csv", extract_centerline(solution)
    )
    write_profile_csv(
        case_dir / f"{case.name}_midplane_y.csv",
        extract_midplane_profile(solution, axis="y", fluid_only=True),
    )
    write_profile_csv(
        case_dir / f"{case.name}_midplane_z.csv",
    ...
    if case_kind == "hartmann":
        ...
        try:
            comparison = closed_channel_validation(
                solution, case_kind, int(ha), reference_root=reference_root
            )
            reference = {
                "available": True,
    ...
    npz_path = save_solution_npz(
        solution,
        case,
        case_dir / f"{case.name}_results.npz",
        case_kind=case_kind,
        ha=ha,
    )
    plot_paths = plot_solution_npz(
        npz_path,
        case_dir / "plots",
        title=f"{case_kind.capitalize()} case (Ha={int(ha)})",
    )
    report = {
        "case": case.name,
        "case_kind": case_kind,
...
def run_movie_case(
    ...
    *,
    ...
    case = make_demo_case(
        case_kind, ha=ha, resolution=resolution, dt=dt, t_final=t_final
    )
    ...
    movie_paths = plot_movie_npz(
        snapshot_npz, case_dir / "movie", stem=f"{case_kind}_startup"
    )
    return {
        "case_kind": case_kind,
        "ha": ha,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run a verbose, teachable LMX theory-meeting demo."
    )
    parser.add_argument(
        "--output", type=Path, default=Path("./artifacts/examples/theory_meeting_demo")
    )
    parser.add_argument("--resolution", type=int, default=32)
    parser.add_argument(
        "--movie-case", choices=["hartmann", "shercliff", "hunt"], default="shercliff"
    )
    ...
    print_block(
        "Input dictionary",
        {
            ...
            else _portable_path(reference_root),
        },
    )

    reports = {
        "hartmann": run_steady_case(
            "hartmann",
            ha=args.hartmann_ha,
            resolution=args.resolution,
            out_dir=args.output,
        ...
        "shercliff": run_steady_case(
            "shercliff",
            ha=args.shercliff_ha,
            resolution=args.resolution,
            out_dir=args.output,
            reference_root=reference_root,
        ),
        "hunt": run_steady_case(
            "hunt",
            ha=args.hunt_ha,
            resolution=args.resolution,
            out_dir=args.output,
    ...
    movie_ha = {
        "hartmann": args.hartmann_ha,
        "shercliff": args.shercliff_ha,
        "hunt": args.hunt_ha,
    }[args.movie_case]
