...
OUTPUT_DIR = Path("artifacts/examples/operator_clustered_verification")
...
def run_operator_clustered_verification_demo() -> dict[str, object]:
    ...
    for resolution in RESOLUTIONS:
        ...
        gradient_y_l2_error = float(
            jnp.sqrt(jnp.mean((grad_y[sl] - exact_grad_y[sl]) ** 2))
        )
        gradient_z_l2_error = float(
            jnp.sqrt(jnp.mean((grad_z[sl] - exact_grad_z[sl]) ** 2))
        )

        lap_field, exact_lap = _laplacian_reference(y, z)
        lap = laplacian_scalar(lap_field, mesh)
        sl_lap = (slice(8, -8), slice(8, -8))
        laplacian_l2_error = float(
            jnp.sqrt(jnp.mean((lap[sl_lap] - exact_lap[sl_lap]) ** 2))
        )
    ...
    plots = write_operator_verification_plots(
        records, OUTPUT_DIR, case_title="LMX clustered-mesh operator verification"
    )

    spacing = jnp.asarray([row["max_spacing"] for row in records], dtype=float)
    grad_y_error = jnp.asarray(
        [row["gradient_y_l2_error"] for row in records], dtype=float
    )
    grad_z_error = jnp.asarray(
        [row["gradient_z_l2_error"] for row in records], dtype=float
    )
    lap_error = jnp.asarray([row["laplacian_l2_error"] for row in records], dtype=float)
    grad_y_order = float(
        jnp.mean(
            jnp.log(grad_y_error[:-1] / grad_y_error[1:])
            / jnp.log(spacing[:-1] / spacing[1:])
        )
    )
    grad_z_order = float(
        jnp.mean(
            jnp.log(grad_z_error[:-1] / grad_z_error[1:])
            / jnp.log(spacing[:-1] / spacing[1:])
        )
    )
    lap_order = float(
        jnp.mean(
            jnp.log(lap_error[:-1] / lap_error[1:])
            / jnp.log(spacing[:-1] / spacing[1:])
        )
    )

    summary = {
        "case": "operator_clustered_verification",
        "mesh": {
            "target_ha": TARGET_HA,
            "magnetic_axis": MAGNETIC_AXIS,
        },
        ...
        "literature_references": [
            "Boundary-layer-resolved clustered-mesh verification for elliptic and gradient operators",
            "Samper et al., verification and validation ladder for fusion MHD codes",
        ],
        "plots": [path.name for path in plots],
    }
    (OUTPUT_DIR / "operator_clustered_verification_summary.json").write_text(
...
if __name__ == "__main__":
    run_operator_clustered_verification_demo()
