...
def plot_solution_npz(
    ...
    *,
    ...
    output_dir = (
        Path(output_dir) if output_dir else npz_path.with_suffix("").parent / "plots"
    )
    ...
    with np.load(npz_path, allow_pickle=False) as data:
        ...
        axes[1, 0].plot(
            y_centers / max(np.max(np.abs(y_centers)), 1e-12),
            mid_y / max(np.max(np.abs(mid_y)), 1e-12),
            color="#0f766e",
        )
        ...
        axes[1, 1].plot(
            z_centers / max(np.max(np.abs(z_centers)), 1e-12),
            mid_z / max(np.max(np.abs(mid_z)), 1e-12),
            color="#0f766e",
        )
        ...
        if "time_history" in data and data["time_history"].size:
            ...
            axes[0].plot(
                time, data["current_max_history"], label="max |J|", color="#b91c1c"
            )
            axes[0].plot(
                time, data["lorentz_max_history"], label="max |JxB|", color="#6d28d9"
            )
            ...
            axes[1].plot(
                time,
                data["potential_residual_history"],
...
def plot_movie_npz(
    ...
    *,
    ...
    npz_path = Path(npz_path)
    output_dir = (
        Path(output_dir) if output_dir else npz_path.with_suffix("").parent / "movie"
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    image = ax2d.pcolormesh(
        z_faces, y_faces, display_stack[0], shading="auto", cmap="magma", norm=norm
    )
    ax2d.set_xlabel("z")
    ax2d.set_ylabel("y")
    ...
    anim2d = animation.FuncAnimation(
        fig2d, update_2d, frames=len(times), interval=1000 / fps, blit=False
    )
    ...
    anim3d = animation.FuncAnimation(
        fig3d, update_3d, frames=len(times), interval=1000 / fps, blit=False
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plot LMX .npz result files with Matplotlib."
    )
    ...
    print(
        json.dumps(
            {
                "npz": _portable_path(args.npz),
                "outputs": [_portable_path(path) for path in outputs],
            },
            indent=2,
        )
    )
