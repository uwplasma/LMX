...
from scripts.run_manual_solver_family_validation import (
    main as run_manual_validation_main,
)
...
def run_extruded_validation_campaign(
    *,
    ...
    if exit_code != 0:
        raise RuntimeError(
            f"manual validation campaign failed with exit code {exit_code}"
        )
    ...
    for row in fringing_rows:
        grouped.setdefault((str(row["geometry_kind"]), float(row["ha"])), []).append(
            row
        )
    ...
    for (geometry_kind, ha_value), rows in sorted(grouped.items()):
        rows = sorted(
            rows, key=lambda row: (float(row["ha"]), float(row["resolution"]))
        )
        resolution = [float(row["resolution"]) for row in rows]
        charge = [
            max(float(row["max_charge_balance_residual"]), 1.0e-16) for row in rows
        ]
        axial_current_span = [
            max(float(row.get("axial_current_span", 0.0)), 1.0e-16) for row in rows
        ]
        peak_velocity_span = [
            max(float(row.get("peak_velocity_span", 0.0)), 1.0e-16) for row in rows
        ]
        pressure_span_range = [
            max(float(row.get("pressure_span_range", 0.0)), 1.0e-16) for row in rows
        ]
    ...
    summary = {
        ...
        "default_fringing_plot": str(
            summary_path.with_name(f"{summary_path.stem}_fringing.png")
        ),
    ...
    (out_dir / "extruded_validation_campaign_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the larger LMX extruded 3D validation campaign."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/examples/extruded_validation_campaign"),
    )
    ...
    parser.add_argument(
        "--fringing-geometries", type=str, default="rect_duct,layered_duct,pipe_ogrid"
    )
