...
from lmx.plotting import write_bent_pipe_overview_plots, write_geometry_preview_plots
...
def run_bent_pipe_inductionless_demo() -> dict[str, object]:
    ...
    (OUTPUT_DIR / "bent_pipe_inductionless_summary.json").write_text(
...
def _write_external_reference_artifacts(
    validation: dict[str, object], bent_solution, out_dir: Path
) -> dict[str, object]:
    reference_path = out_dir / EXTERNAL_REFERENCE_FILENAME
    if not reference_path.exists():
        template_path = write_dean_vortex_reference_template(
    ...
    pressure_span = np.max(
        np.asarray(bent_solution.bundle.p, dtype=float), axis=(1, 2)
    ) - np.min(
