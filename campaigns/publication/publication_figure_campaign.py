...
def run_publication_figure_campaign(
    *,
    ...
    if refresh_fast_figures:
        ...
        refreshed.append("campaigns/blanket/wham_blanket_flow_demo.py")
    ...
    summary = json.loads(
        (out_dir / "publication_figure_campaign_summary.json").read_text(
            encoding="utf-8"
        )
    )
    summary["refreshed_generators"] = refreshed
    summary["docs_artifacts"] = copied
    (out_dir / "publication_figure_campaign_summary.json").write_text(
        json.dumps(summary, indent=2) + "\n", encoding="utf-8"
    )
    if copy_to_docs:
        shutil.copy2(
            out_dir / "publication_figure_campaign_summary.json",
            docs_output_dir / "publication_figure_campaign_summary.json",
        )
