...
def run_q2dmhdfoam_lmx_turbulence_match_audit(
    *,
    ...
    case_dirs = [
        root / relative
        for relative in case_relative_paths
    ...
    strict_admissible_cases = [
        str(audit["case_name"])
        for audit in audits
        if bool(audit.get("strict_admissible", False))
    ]
    summary = {
        "case": "q2dmhdfoam_lmx_turbulence_match_audit",
        ...
        "all_strict_admissible": bool(
            audits and len(strict_admissible_cases) == len(audits)
        ),
        "strict_blocker_closed": False,
...
def _write_summary(
    summary: dict[str, object], out_dir: Path, docs_output_dir: Path | None
) -> Path:
