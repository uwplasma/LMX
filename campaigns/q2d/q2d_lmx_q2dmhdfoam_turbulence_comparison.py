...
from lmx.external_validation import load_q2dmhdfoam_lid_driven_observables
from lmx.q2d import (
...
def run_q2d_lmx_q2dmhdfoam_turbulence_comparison(
    *,
    ...
    plots = _write_comparison_panel(
        solution, validation, q2dmhdfoam_observables, out_dir
    )
...
def _write_comparison_panel(
    ...
    axes[0, 1].plot(
        time,
        energy_history / max(energy_history[0], 1.0e-20),
        color="#0f766e",
        linewidth=2.0,
        label="LMX kinetic energy",
    )
    axes[0, 1].plot(
        time,
        enstrophy_history / max(enstrophy_history[0], 1.0e-20),
        color="#b45309",
        linewidth=2.0,
        label="LMX enstrophy proxy",
    )
    ...
    for key, color, label in (
        ...
        if key in q2dmhdfoam_observables:
            axes[1, 0].axvline(
                float(q2dmhdfoam_observables[key]),
                color=color,
                linestyle="--",
                linewidth=1.2,
                label=label,
    ...
    fig.suptitle(
        "LMX and Q2DmhdFoam Q2D turbulence-observable comparison",
        fontsize=15.5,
        fontweight="bold",
    )
    paths = [
