    ...
    *,
    ...
    if not profiles:
        raise FileNotFoundError(
            f"No Q2DmhdFoam or Vetcha profile samples were found under {root}"
        )

    profile_observables = [
        q2dmhdfoam_profile_observables(profile) for profile in profiles
    ]
    ...
    if turbulence_path.exists():
        turbulence_observables = dict(
            load_q2dmhdfoam_lid_driven_observables(turbulence_path)
        )
    ...
    summary = {
        ...
        "turbulence_observable_table": turbulence_table.name
        if turbulence_table
        else None,
        "timeseries_observable_table": timeseries_table.name
        if timeseries_table
        else None,
    ...
    if copy_to_docs:
        shutil.copy2(
            out_dir / "q2dmhdfoam_external_reference_summary.json",
            docs_output_dir / "q2dmhdfoam_external_reference_summary.json",
        )
    return summary
...
def _write_turbulence_observables_csv(
    observables: dict[str, object], path: Path
) -> Path:
