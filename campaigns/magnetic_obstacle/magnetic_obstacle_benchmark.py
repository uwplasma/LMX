...
def run_magnetic_obstacle_benchmark() -> dict[str, object]:
    ...
    literature_validation = validate_magnetic_obstacle_literature_slice(
    ...
    plots = [
        ...
        *write_extruded_overview_plots(
            solution, OUTPUT_DIR, case_title="Magnetic-obstacle benchmark"
        ),
        *write_magnetic_obstacle_benchmark_plots(
            solution,
            reference_solution,
    ...
    (OUTPUT_DIR / "magnetic_obstacle_benchmark_summary.json").write_text(
...
def _write_external_reference_artifacts(
    lmx_observables: dict[str, float], out_dir: Path
) -> dict[str, object]:
