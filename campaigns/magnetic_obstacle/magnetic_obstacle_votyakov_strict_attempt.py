...
CANDIDATE_REFERENCE_PATH = (
    DOCS_OUTPUT_DIR / "magnetic_obstacle_reference_observables_candidate.csv"
)
...
def run_magnetic_obstacle_votyakov_strict_attempt(
    *,
    ...
    benchmark_summary = json.loads(
        Path(benchmark_summary_path).read_text(encoding="utf-8")
    )
    lmx_observables = dict(
        benchmark_summary.get("external_readiness", {}).get("observables", {})
    )
    if not lmx_observables:
        raise ValueError(
            "magnetic obstacle benchmark summary has no external_readiness observables"
        )

    reference_path = _write_filled_candidate_rows(
        candidate_reference_path, out_dir / STRICT_REFERENCE_FILENAME
    )
    ...
    patched_summary_path.write_text(
        json.dumps(patched_benchmark_summary, indent=2) + "\n", encoding="utf-8"
    )
    summary = {
...
def _write_filled_candidate_rows(candidate_path: Path, output_path: Path) -> Path:
    ...
    with Path(candidate_path).open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            if (row.get("value") or "").strip() and (
                row.get("tolerance") or ""
            ).strip():
                rows.append(row)
    if not rows:
