    ...
    *,
    ...
    for step in range(INVERSE_DESIGN_STEPS):
        ...
        scale = jnp.clip(
            ...
            - NEWTON_RELAXATION
            * (pressure - target)
            / jnp.maximum(jnp.abs(gradient), 1.0e-12),
            0.25,
            16.0,
        )
    pressure, gradient = jax.value_and_grad(pressure_for_scale)(scale)
...
def run_wham_blanket_autodiff_research_demo() -> dict[str, object]:
    ...
    summary = {
        ...
        "d_pressure_drop_d_coil_separation_kpa_per_m": float(
            reference["d_pressure_drop_d_coil_separation"] / 1000.0
        ),
        "d_pressure_drop_d_field_scale_kpa": float(
            reference["d_pressure_drop_d_field_scale"] / 1000.0
        ),
        "d_pressure_drop_d_mean_velocity_kpa_per_m_per_s": float(
            reference["d_pressure_drop_d_mean_velocity"] / 1000.0
        ),
        ...
        "target_pressure_drop_error_kpa": float(
            field_history[-1]["pressure_drop_kpa"] - TARGET_PRESSURE_DROP_KPA
        ),
        "model_limitations": (
    ...
    print(
        f"d(pressure_drop)/d(coil_separation) = {summary['d_pressure_drop_d_coil_separation_kpa_per_m']:.3f} kPa/m"
    )
