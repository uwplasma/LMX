...
def run_wham_mirror_pipe_demo() -> dict[str, object]:
    ...
    problem = replace(
        ...
        profile=replace(
            problem.profile,
            x=solver_x,
            field_scale=jnp.asarray(centerline_scale, dtype=float),
        ),
        case=replace(
            problem.case,
    ...
    autodiff_summary = {
        ...
        "d_pressure_drop_d_separation": float(
            autodiff_reference["d_pressure_drop_d_separation"]
        ),
        "separation_sweep": AUTODIFF_SEPARATION_SWEEP.tolist(),
        "pressure_drop_curve": [
            float(item["pressure_drop_proxy"]) for item in autodiff_sweep
        ],
        "sensitivity_curve": [
            float(item["d_pressure_drop_d_separation"]) for item in autodiff_sweep
        ],
    }
    ...
    summary = {
        ...
        "plots": [
            path.name for path in [*field_plots, *overview_plots, *wham_overview]
        ],
        "field_quality": field_quality,
        "validation": validation,
        "autodiff": autodiff_summary,
    }
    (OUTPUT_DIR / "wham_mirror_pipe_summary.json").write_text(
