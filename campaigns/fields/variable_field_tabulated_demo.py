...
def run_variable_field_tabulated_demo() -> dict[str, object]:
    ...
    reference_solver_field = np.asarray(
        field_fn(jnp.asarray(solver_yy), jnp.asarray(solver_zz)), dtype=float
    )
    ...
    (OUTPUT_DIR / "variable_field_tabulated_summary.json").write_text(
