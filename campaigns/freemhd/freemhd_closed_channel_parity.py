...
from lmx.plotting import write_freemhd_parity_plots
...
def _run_case(case_kind: str, ha: float, reference_run_dir: Path) -> dict[str, object]:
    ...
    u_max_abs_diff = abs(
        lmx_u_max - float(latest_u.max_value if latest_u is not None else lmx_u_max)
    )
    freemhd_history_records = []
    for path in reference_run_dir.glob("postProcessing/**/fieldMinMax.dat"):
        freemhd_history_records.extend(
            record for record in read_field_minmax(path) if record.field == "mag(U)"
        )
    ...
    return {
        ...
        "freemhd_execution_seconds": float(
            parse_freemhd_execution_seconds(reference_run_dir / "run.log") or 0.0
        ),
...
def run_freemhd_closed_channel_parity() -> dict[str, object]:
    ...
    case_audits = {
        case_kind: audit_freemhd_case_against_spec(
            FREEMHD_INSTALL_DIR / "freemhd_output" / case_kind,
            case_kind=case_kind,
        )
        for case_kind, _ in CASES
    }
    if not all(bool(audit["matched"]) for audit in case_audits.values()):
        summary = {
            "case": "freemhd_closed_channel_parity",
            "status": "invalid_reference",
            "reason": "FreeMHD inputs do not match the canonical Benchmark-A specifications.",
        ...
        (OUTPUT_DIR / "freemhd_closed_channel_parity_summary.json").write_text(
    ...
    plots = write_freemhd_parity_plots(
        records, OUTPUT_DIR, case_title="LMX vs FreeMHD closed-channel parity"
    )
    summary = {
        "case": "freemhd_closed_channel_parity",
        "status": "completed",
        "case_audits": case_audits,
        "records": records,
        "top_profile_offenders": summarize_profile_error_offenders(
            records, l2_target=1.0e-2, top_n=4
        ),
