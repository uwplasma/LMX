...
def run_autodiff_sensitivity_demo(
    *,
    ...
    values = (
        hartmann_values if hartmann_values is not None else jnp.linspace(2.0, 28.0, 20)
    )
    ...
    for ha in values:
        autodiff = hartmann_mean_velocity_gradients(
            problem, forcing=forcing, hartmann_number=ha
        )
    ...
    fd_grad_forcing = np.asarray(
        [item["fd_d_mean_velocity_d_forcing"] for item in records]
    )

    axes[0].plot(ha, grad_ha, color="#0f766e", label="autodiff")
    axes[0].plot(
        ha, fd_grad_ha, color="#b45309", linestyle="--", label="finite difference"
    )
    ...
    axes[1].plot(
        ha, fd_grad_forcing, color="#7c3aed", linestyle="--", label="finite difference"
    )
    ...
    (out_dir / "autodiff_sensitivity_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate LMX autodiff sensitivities against finite differences."
    )
    parser.add_argument(
        "--output", type=Path, default=Path("artifacts/examples/autodiff_sensitivity")
    )
