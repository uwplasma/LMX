...
def run_autodiff_extruded_trajectory_demo(
    *,
    ...
    target_u = np.asarray(target["u_field"][:, 0]).reshape(
        target["u_field"].shape[0], -1
    )
    recovered_u = np.asarray(result["recovered_u_history"][:, 0]).reshape(
        target["u_field"].shape[0], -1
    )
    target_phi = np.asarray(target["phi_field"][:, -1]).reshape(
        target["phi_field"].shape[0], -1
    )
    recovered_phi = np.asarray(result["recovered_phi_history"][:, -1]).reshape(
        target["phi_field"].shape[0], -1
    )
    ...
    axes[0, 1].plot(
        iterations,
        np.linalg.norm(target_u - recovered_u, axis=1),
        marker="o",
        color="#0f766e",
    )
    ...
    axes[1, 0].plot(
        iterations,
        np.linalg.norm(target_phi - recovered_phi, axis=1),
        marker="o",
        color="#b45309",
    )
    ...
    axes[1, 1].plot(
        iterations,
        np.asarray(target["charge_balance_residual"][:, 0]),
        label="Target",
        color="#0891b2",
    )
    axes[1, 1].plot(
        iterations,
        np.asarray(result["recovered_charge_balance_history"][:, 0]),
        label="Recovered",
        color="#dc2626",
        linestyle="--",
    )
    ...
    (out_dir / "autodiff_extruded_trajectory_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the LMX projection-trajectory extruded autodiff demo."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/examples/autodiff_extruded_trajectory"),
    )
