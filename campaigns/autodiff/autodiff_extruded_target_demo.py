...
def run_autodiff_extruded_target_demo(
    *,
    ...
    axes[0, 0].plot(
        x,
        np.asarray(recovered["recovered_mean_velocity"]),
        label="Recovered",
        color="#b45309",
        linestyle="--",
        linewidth=2.0,
    ...
    axes[0, 1].plot(
        x,
        np.asarray(target["current_proxy"]),
        label="Target",
        color="#1d4ed8",
        linewidth=2.0,
    )
    axes[0, 1].plot(
        x,
        np.asarray(recovered["recovered_current_proxy"]),
        label="Recovered",
        color="#dc2626",
        linestyle="--",
        linewidth=2.0,
    ...
    axes[1, 1].semilogy(
        x,
        np.maximum(np.asarray(target["charge_balance_residual"]), 1.0e-16),
        label="Charge balance",
        color="#7c3aed",
    )
    axes[1, 1].semilogy(
        x,
        np.maximum(np.asarray(target["wall_current_leakage"]), 1.0e-16),
        label="Wall leakage",
        color="#dc2626",
        linestyle="--",
    )
    axes[1, 1].plot(
        x, np.asarray(target["pressure_span"]), label="Pressure span", color="#0891b2"
    )
    ...
    summary = {
        ...
        "target": {
            ...
            "charge_balance_residual": np.asarray(
                target["charge_balance_residual"]
            ).tolist(),
            ...
            "transverse_kinetic_energy": np.asarray(
                target["transverse_kinetic_energy"]
            ).tolist(),
        },
        "recovered": {
            ...
            "history": recovered["history"],
            "pressure_span": np.asarray(
                recovered.get("recovered_pressure_span", [])
            ).tolist(),
            "transverse_kinetic_energy": np.asarray(
                recovered.get("recovered_transverse_kinetic_energy", [])
            ).tolist(),
        },
        "plots": [png_path.name, pdf_path.name],
    }
    (out_dir / "autodiff_extruded_target_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the LMX extruded-target autodiff inverse-design demo."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/examples/autodiff_extruded_target"),
    )
    parser.add_argument(
        "--geometry-kind", choices=("rect_duct", "pipe_ogrid"), default="rect_duct"
    )
