...
def run_autodiff_profile_design_demo(
    *,
    ...
    axes[2].plot(
        y,
        np.asarray(design["recovered_profile"]),
        color="#7c3aed",
        linestyle="--",
        label="recovered profile",
    )
    ...
    (out_dir / "autodiff_profile_design_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the LMX autodiff full-profile inverse-design demo."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/examples/autodiff_profile_design"),
    )
