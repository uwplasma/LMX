...
def run_autodiff_fringing_response_demo(
    *,
    ...
    axes[0, 1].plot(
        x,
        np.asarray(result["recovered_current_proxy"]),
        label="Recovered",
        color="#dc2626",
        linestyle="--",
        linewidth=2.0,
    ...
    (out_dir / "autodiff_fringing_response_summary.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the LMX fringing multi-observable autodiff inverse-design demo."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/examples/autodiff_fringing_response"),
    )
