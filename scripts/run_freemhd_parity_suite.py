...
DEFAULT_FREEMHD_SOURCE_REPO = Path(
    "/Users/rogerio/local/tests/lmx_external_codes/FreeMHD"
)
...
def audit_freemhd_case_against_spec(
    ...
    *,
    ...
    def check(name: str, expected: object, observed: object) -> dict[str, object]:
        passed = (
            math.isclose(
                float(observed), float(expected), rel_tol=1.0e-9, abs_tol=1.0e-14
            )
    ...
    if mesh is not None and (
        match := re.search(r"\bHa\s+([0-9eE+.\-]+)\s*;", mesh.read_text())
    ):
    ...
    checks += [
        ...
        check(
            "physics.hartmann", spec["magnetic_field"]["hartmann_number"], physical_ha
        ),
        check(
...
def materialize_freemhd_source_snapshot(
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing FreeMHD source snapshot {destination}"
        )
    reference = load_benchmark_b_spec(case_id, spec_root)[
        "free_mhd_discretization_reference"
    ]
    ...
    for name in _FREEMHD_SOURCE_NAMES:
        ...
        try:
            ...
            raise ValueError(
                f"FreeMHD source is missing or escapes its repository: {relative}"
            ) from exc
        ...
        if tracked.returncode or not tracked.stdout.startswith("100"):
            raise ValueError(
                f"FreeMHD source is not a tracked regular file: {relative}"
            )
    ...
    for relative, expected in files.items():
        if artifact_sha256(repository / relative, "file") != expected:
            raise ValueError(
                f"FreeMHD source SHA-256 does not match the frozen specification: {relative}"
            )
...
def materialize_lmx_source_snapshot(output_dir: str | Path) -> dict[str, object]:
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing LMX source snapshot {destination}"
        )
...
def _tiny_b2_problem(
    ...
    problem = build_benchmark_b_problem(
        "B2-fringing-square", mesh_level="coarse", root=spec_root
    )
    dt = 1.0 / 540000.0
    case = replace(
        problem.case,
        ...
        geometry=replace(
            problem.case.geometry, nx=8, ny=5, nz=5, wall_cells=(1, 1, 1, 1)
        ),
        time_stepper=replace(
            problem.case.time_stepper, dt=dt, t_final=2.0 * dt, max_steps=2
        ),
    )
    ...
    payload: dict[str, object] = {
        ...
        "mesh": {
            ...
            **{
                f"{axis}_faces": np.asarray(
                    getattr(mesh, f"{axis}_faces"), dtype=float
                ).tolist()
                for axis in "xyz"
            },
        },
...
def materialize_matched_b2_lmx_input(
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing LMX B2 input {destination}"
        )
...
def materialize_matched_b2_evaluator(output_file: str | Path) -> dict[str, object]:
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing matched B2 evaluator {destination}"
        )
    payload = {
        "schema_version": 1,
        "case_id": "B2-fringing-square",
...
def _foam_text(
    object_name: str, body: str, *, class_name: str = "dictionary", location: str = ""
) -> str:
...
def _b2_field_expression(anchor_count: int) -> str:
    ...
    slopes = [
        f"(b{right}-b{left})/(x{right}-x{left})"
...
def materialize_matched_b2_freemhd_input(
    ...
    *,
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing FreeMHD B2 input {destination}"
        )
    missing = [
        relative for relative in _B2_SKELETON_FILES if not (source / relative).is_file()
    ]
    if missing:
        raise ValueError(f"FreeMHD hunt_demo skeleton is incomplete: {missing}")
    skeleton_hashes = {
        relative: artifact_sha256(source / relative, "file")
        for relative in _B2_SKELETON_FILES
    }
    destination.mkdir(parents=True)
    ...
    _write_foam(
        destination / "constant/liquid/turbulenceProperties", "simulationType laminar;"
    )
    _write_foam(
...
"""
    _write_foam(
        destination / "constant/liquid/thermophysicalProperties.liquidMetal", fluid_body
    )
    _write_foam(
        destination / "constant/liquid/thermophysicalProperties.air",
        fluid_body.replace(
            "elcond [-1 -3 3 0 0 2 0] 1;", "elcond [-1 -3 3 0 0 2 0] 0;"
        ),
    )
    solid_body = """
...
"""
    _write_foam(
        destination / "constant/solidWalls/thermophysicalProperties", solid_body
    )
...
def run_matched_b2_freemhd_smoke(
    ...
    *,
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing FreeMHD output {destination}"
        )
...
"""
    ...
    try:
        try:
            ...
            raise TimeoutError(
                f"FreeMHD B2 smoke exceeded {timeout_seconds:g} seconds"
            ) from error
    finally:
        try:
            subprocess.run(
...
def run_matched_b2_smoke_bundle(
    ...
    *,
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing matched B2 bundle {destination}"
        )
    ...
    lmx = run_matched_b2_lmx_smoke(
        paths["lmx_input"], paths["evaluator"], paths["lmx_output"]
    )
    ...
    lmx_failed = (
        ...
        or any(
            abs(value - 1.0 / 540000.0) > limits["dt_absolute_tolerance"]
            for value in lmx["dt"]
        )
    ...
    if lmx_failed:
        raise ValueError(
            "LMX B2 smoke failed its frozen execution gate; FreeMHD was not started"
        )
    ...
    spec_path = (
        Path(__file__).resolve().parents[1] / "benchmarks/specs/alex-b2-square.toml"
    )
    record = {
        "schema_version": 3,
        "case_id": "B2-fringing-square",
...
def _replace(
    path: Path, pattern: str, replacement: str, *, required: bool = True
) -> int:
    updated, count = re.subn(pattern, replacement, path.read_text(encoding="utf-8"))
    if required and count == 0:
        raise ValueError(
            f"Expected input was not found in FreeMHD template file {path}"
        )
    if count:
...
def materialize_matched_freemhd_case(
    ...
    *,
    ...
    if destination.exists():
        raise FileExistsError(
            f"Refusing to overwrite existing FreeMHD case {destination}"
        )
    spec = load_benchmark_a_spec(case_kind, spec_dir)
    shutil.copytree(source, destination)
    vector = " ".join(
        f"{float(value):.16g}" for value in spec["magnetic_field"]["vector"]
    )
    ...
    changed[liquid.relative_to(destination).as_posix()] = sum(
        _replace(liquid, pattern, rf"\g<1>{value:.16g}\g<2>")
    ...
    audit = audit_freemhd_case_against_spec(
        destination, case_kind=case_kind, spec_dir=spec_dir
    )
    if not audit["matched"]:
        failures = [check["name"] for check in audit["checks"] if not check["pass"]]
        raise ValueError(
            f"Generated FreeMHD case failed its canonical audit: {failures}"
        )
    manifest = {
        "schema_version": 1,
        "case_kind": case_kind,
...
def _max_metric(
    records: list[dict[str, Any]], path: tuple[str | None, ...]
) -> float | None:
...
def run_suite(
    *,
    ...
    has_transient_reference = all(
        (reference_output_root / name).exists() for name in ("shercliff", "hunt")
    )
    ...
    summary: dict[str, Any] = {
        "status": "completed",
        "reason": "",
        "case_dir": str(freemhd_install_dir)
    ...
    if has_transient_reference:
        ...
        summary["matched_case_gate"] = all(
            bool(audit["matched"]) for audit in case_audits.values()
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run available FreeMHD parity artifact checks."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            os.environ.get("LMX_FREEMHD_INSTALL_DIR", DEFAULT_FREEMHD_INSTALL_DIR)
        ),
    )
    parser.add_argument(
        "--processed-root",
        type=Path,
        default=Path(
            os.environ.get("LMX_FREEMHD_PROCESSED_ROOT", DEFAULT_PROCESSED_ROOT)
        ),
    )
    parser.add_argument(
        "--freemhd-source-repo",
        type=Path,
        default=Path(
            os.environ.get("LMX_FREEMHD_SOURCE_REPO", DEFAULT_FREEMHD_SOURCE_REPO)
        ),
    )
    ...
    if args.matched_b2_smoke:
        ...
        return (
            0 if report.get("execution_pass") and report.get("comparison_pass") else 2
        )
