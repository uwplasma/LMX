...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run native LMX pseudo-time convergence studies.")
    parser.add_argument("--output", type=Path, default=Path("artifacts/time_convergence"))
    ...
    dts = _parse_csv_floats(args.dts)
    payload: dict[str, object] = {"ha": args.ha, "resolution": args.resolution, "cases": {}}

    for case_kind in cases:
        levels: list[dict[str, float | str]] = []
        for dt in dts:
            case_dir = args.output / case_kind / f"dt{dt:g}"
            case = _build_case(case_kind, args.ha, args.resolution, case_dir)
            t_final = case.time_stepper.t_final if args.t_final is None else float(args.t_final)
            ...
            case = _replace_like(
                case,
                time_stepper=_replace_like(case.time_stepper, dt=float(dt), t_final=float(t_final), max_steps=max_steps),
            )
