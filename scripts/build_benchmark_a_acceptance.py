...
def build_acceptance(results_dir: Path) -> dict[str, Any]:
    ...
    table_filename = "samper-table-i-accepted.json"
    table_path = results_dir / table_filename
    table = _load(table_path)
    table_implementation = _implementation(table, table_path)
    records = table.get("records")
    if not table.get("research_grade_validation_pass") or not isinstance(records, list):
        raise ValueError(f"Table I evidence is not a passing campaign: {table_path}")
    indexed = {
        (str(record.get("case_kind")), int(record.get("hartmann_number", -1))): record
        for record in records
    }
    expected = {(case, ha) for case in CASES for ha in HARTMANN_NUMBERS}
    if len(indexed) != len(records) or set(indexed) != expected:
        raise ValueError("Table I evidence does not contain the exact eight-row matrix")
    if _solver_identity(table_implementation) != solver_identity:
    ...
    for case in CASES:
        for ha in HARTMANN_NUMBERS:
            record = indexed[(case, ha)]
            if not record.get("finest_level_pass"):
                raise ValueError(f"Table I finest-level gate failed: {case} Ha={ha}")
            ...
            table_rows.append(
                {
                    ...
                    "source": f"{table_filename}#{case}-ha{ha}",
    ...
    implementation = {
        **dict(solver_identity),
        "runners": {
            ...
            "table_i_sha256": table_implementation["runner_sha256"],
        },
    }
