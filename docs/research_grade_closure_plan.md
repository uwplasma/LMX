...
The heavy external-code runs should remain opt-in. Routine CI/CD should still
carry bounded validation, so the push and release workflows now run parallel
solver-summary, mesh/time-convergence, Q2D reduced-model, and
external-reference contract jobs under the 10-minute wall-clock target. Heavy
campaigns regenerate external data and publication figures only when explicitly
requested.
...
`campaigns/walls/li_aln_wall_stack_phase0_2.py` writes the unit, mesh-QA, conductance,
and pinhole-sensitivity artifact. `campaigns/walls/li_aln_wall_stack_phase3_6.py`
writes the bounded operating matrix, substrate comparison, degradation
thresholds, and pinhole-limit artifact. `campaigns/walls/li_aln_multilayer_mesh_qa.py`
...
- Q2D turbulence targets should be anchored to Sommeria-Moreau, Vetcha-
  ...
  summary. `campaigns/q2d/q2dmhdfoam_lmx_turbulence_match_audit.py` now records that
...
2. Before promoting a run, pass
   `campaigns/q2d/q2dmhdfoam_lmx_turbulence_match_audit.py`. This audit is the
...
4. Extend `campaigns/q2d/q2dmhdfoam_external_reference_adapter.py` if needed so it
   writes `q2d_turbulence_reference_observables.csv` directly for the matched
   case.
5. Run `campaigns/q2d/q2d_turbulence_decay_demo.py` with the filled CSV present so
...
3. If executable parity is not immediately available, digitize published
   ...
   metadata.
4. Use `campaigns/magnetic_obstacle/magnetic_obstacle_votyakov_curve_validation.py` to check the
...
5. Rerun `campaigns/fringing/bent_pipe_inductionless_demo.py` with the filled CSV and
...
- Add a `docs/_static/generated/research_grade_closure_status.json` file that
  ...
  `converged`, or `closed`.
- Keep this phase lightweight in CI: validate schema and artifact presence,
  plus bounded solver/Q2D/convergence artifacts, not heavy external-code
  execution.

Current status: the closure-status artifact is generated by
`campaigns/status/research_grade_closure_status.py` and copied to
...
`campaigns/status/research_grade_external_data_audit.py`. That artifact checks which
...
`campaigns/status/research_grade_external_target_figures.py` now writes
...
The publication-facing closure dashboard is generated by
`campaigns/status/research_grade_closure_dashboard.py`. It combines the closed
Q2DmhdFoam side-wall support gate with the explicit Votyakov magnetic-obstacle
and Bayat-Rezai Dean-vortex strict mismatches plus the current closure ledger:

![Strict research-grade validation closure dashboard](https://github.com/uwplasma/LMX/releases/download/lmx-research-assets-v1/research_grade_closure_dashboard.png)

The final strict-lane disposition is generated by
`campaigns/status/research_grade_final_lane_disposition.py`. It is the last-push audit
record: it keeps the strict lanes deferred when the measured external or
physics gates fail and records the exact offender that must be addressed next.

![Final strict research-lane disposition](https://github.com/uwplasma/LMX/releases/download/lmx-research-assets-v1/research_grade_final_disposition.png)
...
- Q2D: produce one matched Q2DmhdFoam turbulent reference bundle and convert it
  ...
  `campaigns/q2d/q2dmhdfoam_lid_driven_vtk_artifact.py` for field-level velocity and
  vorticity observables. The matching LMX side-wall comparison now exists in
  `campaigns/q2d/q2d_lmx_q2dmhdfoam_lid_driven_parity.py`; it uses an isothermal
...
- Magnetic obstacle: either build a reproducible MHD_Solvers_OpenFOAM localized
  ...
  `campaigns/magnetic_obstacle/magnetic_obstacle_votyakov_strict_attempt.py`; it compares the
...
- Dean: digitize one Dean-flow reference set or generate one OpenFOAM curved-
  ...
  `campaigns/fringing/dean_vortex_bayat_rezai_strict_attempt.py`; it compares the current
...
- Update `campaigns/publication/publication_figure_campaign.py` so the manifest reports
