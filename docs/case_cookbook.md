...
Current blocker: the Docker daemon is reachable on this machine, and the recovered `Ha20` case is now discoverable, but the OpenFOAM base image/build path is still not proven end to end because `lmx-freemhd` has not yet been built successfully in this session. The current container preflight report shows both that the requested local image tag is missing and that `docker manifest inspect openfoam/openfoam2206-paraview:latest` times out from this environment.
