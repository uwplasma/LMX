...
Current status: the Docker path is now proven for short smoke runs on this machine when forced to `linux/amd64`. The recovered Shercliff `Ha20` case runs through `epotMultiRegionInterFoam`, reconstructs `0.0001/`, emits `postProcessing/liquid/minMax/0/fieldMinMax.dat`, and now also emits sampled line cuts under `postProcessing/<dict-name>/liquid/0.0001/` through `scripts/sample_freemhd_profiles.py`. The sampling script now infers the line-cut extents automatically from the latest `fieldMinMax.dat` record when explicit `x/y/z` limits are omitted. `scripts/run_freemhd_parity_report.py` then wraps the current LMX-side case build plus `compare_with_freemhd` into a single JSON artifact. On the non-Docker path, the Darwin patch helper still removes the libc++ header collision and advances the local `wmake` failure to a new `fvMesh.H` include regression, which remains machine-readable.
