...
The current checkpoint records 35 package modules and 34,425 total
...
The current tracked checkout is 4,550,352 bytes, below its 4.5 MiB hard cap.
...
SHA-256 verification. Five compressed web derivatives (168,237 bytes total) remain in
...
- Reference coverage lane: 863 tests pass with 95.40% combined line/branch
  coverage over `lmx/` in 121.5 seconds; workflow behavior is exercised by the
  same suite
