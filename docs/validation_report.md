...
- The current Docker/image blocker has narrowed to image availability and local pull/build execution: the helper now reports `docker-image-unavailable` cleanly when the requested tag does not exist locally.
- The container bundle now uses `microfluidica/openfoam:2206`, which the Docker Hub tag API confirms is valid from this environment.
- The current container preflight on this machine still shows the base-image pull itself timing out, so the unresolved Docker-side problem is now local pull/build execution rather than tag discovery.
