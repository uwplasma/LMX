...
def run_magnetic_obstacle_regime_scan() -> dict[str, object]:
            ...
            records.append(
                {
                    ...
                    "peak_velocity_deficit_ratio": float(validation["peak_velocity_deficit_ratio"]),
    ...
    plots = write_magnetic_obstacle_regime_plots(records, OUTPUT_DIR, case_title="Magnetic-obstacle regime scan")
    summary = {
        "case": "magnetic_obstacle_regime_scan",
        "plots": [path.name for path in plots],
        "records": records,
    }
    (OUTPUT_DIR / "magnetic_obstacle_regime_scan_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    return summary
