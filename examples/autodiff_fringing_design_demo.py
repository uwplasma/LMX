...
def run_autodiff_fringing_design_demo(
    *,
    ...
    problem = build_fringing_autodiff_problem(nx_stations=15, ny=12, nz=12, macro_iterations=3, potential_iterations=12, velocity_iterations=16)
    ...
    axes[0, 0].plot(x, np.asarray(target["field_scale"]), label="Target", color="#1d4ed8", linewidth=2.0)
    axes[0, 0].plot(x, np.asarray(result["recovered_field_scale"]), label="Recovered", color="#dc2626", linestyle="--", linewidth=2.0)
    axes[0, 0].set_title("Fringing field scale")
    axes[0, 0].set_xlabel("x")
    axes[0, 0].set_ylabel(r"$B/B_{peak}$")
    axes[0, 0].legend(frameon=False)

    axes[0, 1].plot(x, np.asarray(target["mean_velocity"]), label="Target", color="#0f766e", linewidth=2.0)
    axes[0, 1].plot(x, np.asarray(result["recovered_mean_velocity"]), label="Recovered", color="#b45309", linestyle="--", linewidth=2.0)
    ...
    (out_dir / "autodiff_fringing_design_summary.json").write_text(json.dumps(summary, indent=2))
    return summary


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the LMX fringing-profile autodiff inverse-design demo.")
    parser.add_argument("--output", type=Path, default=Path("artifacts/examples/autodiff_fringing_design"))
