...
from lmx import generate_rect_duct_mesh, write_interface_verification_plots
...
def _exact_piecewise_linear_solution(y: jnp.ndarray) -> tuple[jnp.ndarray, float]:
    left_length = 1.0
    right_length = 1.0
    flux = (BOUNDARY_RIGHT - BOUNDARY_LEFT) / (left_length / SIGMA_LEFT + right_length / SIGMA_RIGHT)
...
def _solve_interface_problem(mesh) -> tuple[np.ndarray, float]:
    ...
    left_flux = interface[interface_index] * (numeric[interface_index + 1] - numeric[interface_index])
...
def run_interface_conductivity_verification_demo() -> dict[str, object]:
    ...
    for resolution in RESOLUTIONS:
        ...
        profile_l2_error = float(jnp.sqrt(jnp.mean((jnp.asarray(numeric) - exact) ** 2)))
    ...
    profile_error = jnp.asarray([row["profile_l2_error"] for row in records], dtype=float)
    flux_error = jnp.asarray([row["flux_error"] for row in records], dtype=float)
    if bool(jnp.all(profile_error > 1e-12)):
        profile_order = float(jnp.mean(jnp.log(profile_error[:-1] / profile_error[1:]) / jnp.log(spacing[:-1] / spacing[1:])))
    else:
        profile_order = None
    if bool(jnp.all(flux_error > 1e-12)):
        flux_order = float(jnp.mean(jnp.log(flux_error[:-1] / flux_error[1:]) / jnp.log(spacing[:-1] / spacing[1:])))
    else:
        flux_order = None
    ...
    (OUTPUT_DIR / "interface_conductivity_verification_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    return summary
