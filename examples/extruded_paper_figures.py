...
def _write_geometry_3d_plot(solution, out_dir: Path, *, title: str, stem: str) -> list[str]:
    ...
    surface = ax3d.plot_surface(
        ...
        y_grid,
        z_grid,
        facecolors=plt.cm.viridis((u_station - u_station.min()) / max(np.ptp(u_station), 1.0e-12)),
        rstride=1,
        cstride=1,
        shade=False,
        antialiased=True,
    ...
    im_xy = ax_xy.pcolormesh(x_grid, y_grid_surface, u_xymid, shading="auto", cmap="viridis")
    plt.colorbar(im_xy, ax=ax_xy, fraction=0.046, pad=0.04)
    ax_xy.set_title("Midplane velocity u(x, y, zmid)")
    ax_xy.set_xlabel("x")
    ax_xy.set_ylabel("y")

    im_xz = ax_xz.pcolormesh(x_grid_z, z_grid_surface, u_xzmid, shading="auto", cmap="viridis")
...
def run_extruded_paper_figures(
    *,
    ...
    for solution, label, color in (
        ...
        axes[0, 0].plot(x, np.asarray(solution.bundle.mean_velocity), label=label, color=color)
        axes[0, 1].semilogy(x, np.maximum(np.asarray(solution.bundle.charge_balance_residual), 1.0e-16), label=label, color=color)
        axes[1, 0].plot(x, np.asarray(solution.bundle.axial_current), label=label, color=color)
