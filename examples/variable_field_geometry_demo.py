...
from lmx.specs import GeometrySpec, MagneticFieldSpec
...
def _analytic_cross_section_field(width: float, height: float, base_bz: float):
    def field(y: jnp.ndarray, z: jnp.ndarray) -> jnp.ndarray:
        ...
        bz = base_bz * (1.0 + 0.15 * jnp.cos(jnp.pi * y_hat) * jnp.cos(0.5 * jnp.pi * z_hat))
...
def run_variable_field_geometry_demo(*, out_dir: Path) -> dict[str, object]:
    ...
    metrics = validation_summary(solution, rect_case.name, ha=rect_case.geometry.target_ha)
    summary = {
        "rectangular_case": {
            "name": rect_case.name,
    ...
    (out_dir / "variable_field_geometry_summary.json").write_text(json.dumps(summary, indent=2))
    return summary


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the LMX variable magnetic-field and geometry demo.")
    parser.add_argument("--output", type=Path, default=Path("artifacts/examples/variable_field_geometry"))
