...
EXTERNAL_REFERENCE_TEMPLATE_FILENAME = "q2d_turbulence_reference_observables_template.csv"
...
def run_q2d_turbulence_decay_demo() -> dict[str, object]:
    ...
    external_reference_comparison = _write_external_reference_artifacts(validation, OUTPUT_DIR)
    summary = {
...
def _write_external_reference_artifacts(validation: dict[str, object], out_dir: Path) -> dict[str, object]:
    reference_path = out_dir / EXTERNAL_REFERENCE_FILENAME
    if not reference_path.exists():
        template_path = write_q2d_turbulence_reference_template(out_dir / EXTERNAL_REFERENCE_TEMPLATE_FILENAME)
        return {
