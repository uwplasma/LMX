...
def run_autodiff_extruded_field_design_demo(
    *,
    ...
    def _panel(ax, target_field, recovered_field, title: str, cmap: str) -> None:
        im = ax.imshow(np.asarray(target_field - recovered_field), origin="lower", cmap=cmap, extent=extent)
        ax.set_title(title)
        ax.set_xlabel("z")
        ax.set_ylabel("y")
    ...
    (out_dir / "autodiff_extruded_field_design_summary.json").write_text(json.dumps(summary, indent=2))
    return summary


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the LMX field-level extruded autodiff inverse-design demo.")
    parser.add_argument("--output", type=Path, default=Path("artifacts/examples/autodiff_extruded_field_design"))
