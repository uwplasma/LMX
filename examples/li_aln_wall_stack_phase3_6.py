...
from lmx import LithiumMaterial, WallStackStudyCase, write_li_aln_phase3_6_artifacts
...
ALN_CONDUCTIVITIES_S_M = (1.0e-10, 1.0e-9, 1.0e-8, 1.0e-7, 1.0e-6, 1.0e-5, 1.0e-4, 1.0e-3)
...
def run_li_aln_wall_stack_phase3_6(
    *,
    ...
    ten_pct = [
        ...
        if abs(float(row["tolerance_fraction"]) - 0.10) < 1.0e-12 and row["substrate"] == "316L"
    ][0]
    print(f"Li/AlN Phase 3-6 artifacts written to {output_dir}")
    print(f"maximum 316L pinhole fraction for 10% deviation = {float(ten_pct['maximum_pinhole_fraction']):.3e}")
    print("Scope: MHD electrical performance only; true multilayer geometry remains a solver-extension lane.")
    return summary
