...
def run_variable_field_validation() -> dict[str, object]:
    ...
    metrics = cross_section_divergence_metrics(field_fn, width=WIDTH, height=HEIGHT, ny=NY, nz=NZ)
    metrics_path = save_cross_section_divergence_report(metrics, OUTPUT_DIR / "field_divergence_metrics.json")
    summary = {
    ...
    (OUTPUT_DIR / "variable_field_validation_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    return summary
