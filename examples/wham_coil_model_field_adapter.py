...
def run_wham_coil_model_field_adapter(
    *,
    ...
    if not script.exists():
        ...
        (out_dir / "wham_coil_model_field_adapter_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
        return summary
...
def _write_wham_coil_model_plot(params: dict[str, object], out_dir: Path) -> list[Path]:
    ...
    contour = axes[0].contourf(CONTOUR_X, CONTOUR_Z, bmag.T, levels=levels, cmap="magma")
    axes[0].axhline(float(params["coil_center_negative"]), color="white", linestyle="--", linewidth=1.0)
    axes[0].axhline(float(params["coil_center_positive"]), color="white", linestyle="--", linewidth=1.0)
    ...
    axes[1].plot(axis_z, axis_b / max(float(np.max(axis_b)), 1.0e-12), color="#0f766e", linewidth=2.0)
    axes[1].axvline(float(params["coil_center_negative"]), color="#111827", linestyle="--", linewidth=1.0)
    axes[1].axvline(float(params["coil_center_positive"]), color="#111827", linestyle="--", linewidth=1.0)
