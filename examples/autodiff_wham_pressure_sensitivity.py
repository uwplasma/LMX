...
def run_autodiff_wham_pressure_sensitivity() -> dict[str, object]:
    ...
    pressure_proxy = np.asarray([float(item["pressure_drop_proxy"]) for item in sweep], dtype=float)
    pressure_gradient = np.asarray([float(item["d_pressure_drop_d_separation"]) for item in sweep], dtype=float)
    ...
    axes[0].plot(x, np.asarray(reference["field_scale"], dtype=float), color="#1d4ed8", label=r"$B/B_{max}$")
    axes[0].plot(
        x,
        np.asarray(reference["pressure_span"], dtype=float) / max(float(np.max(np.asarray(reference["pressure_span"], dtype=float))), 1.0e-12),
        color="#b91c1c",
    ...
    axes[1].axvline(REFERENCE_SEPARATION, color="#111827", linestyle="--", linewidth=1.0)
    ...
    axes[2].axvline(REFERENCE_SEPARATION, color="#111827", linestyle="--", linewidth=1.0)
    ...
    summary = {
        ...
        "d_pressure_drop_d_separation": float(reference["d_pressure_drop_d_separation"]),
    ...
    (OUTPUT_DIR / "autodiff_wham_pressure_sensitivity_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    return summary
