...
def run_dean_vortex_bayat_rezai_strict_attempt(
    *,
    ...
    reference_path = _write_bayat_rezai_reference_rows(out_dir / STRICT_REFERENCE_FILENAME)
    reference_observables = load_scalar_reference_observables(reference_path, context="Dean-vortex reference CSV")
    lmx_observables = {
        key: float(value)
    ...
    patched_summary_path.write_text(json.dumps(patched_bent_summary, indent=2) + "\n", encoding="utf-8")
...
def _write_bayat_rezai_reference_rows(path: Path) -> Path:
    ...
    field = dean_secondary_flow_field(y, z, tube_radius=1.0, target_rms_velocity=dean_velocity)
