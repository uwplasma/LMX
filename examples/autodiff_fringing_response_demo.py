...
def run_autodiff_fringing_response_demo(
    *,
    out_dir: Path,
    forcing: float = 1.0,
    ...
    fig.suptitle("LMX autodiff fringing multi-observable inverse design", fontsize=16)
    x = np.asarray(result["x"])

    axes[0, 0].plot(x, np.asarray(target["mean_velocity"]), label="Target", color="#0f766e", linewidth=2.0)
    axes[0, 0].plot(x, np.asarray(result["recovered_mean_velocity"]), label="Recovered", color="#b45309", linestyle="--", linewidth=2.0)
    ...
    axes[0, 1].plot(x, np.asarray(result["recovered_current_proxy"]), label="Recovered", color="#dc2626", linestyle="--", linewidth=2.0)
    axes[0, 1].set_title("Current proxy history")
    axes[0, 1].set_xlabel("x")
    axes[0, 1].set_ylabel(r"$\langle |J_y| \rangle$")
    ...
    axes[1, 1].plot([item["iteration"] for item in history], [item["peak_hartmann_number"] for item in history], label="Peak Ha", color="#1d4ed8")
    axes[1, 1].plot([item["iteration"] for item in history], [item["entry_center"] for item in history], label="Entry", color="#0f766e")
    axes[1, 1].plot([item["iteration"] for item in history], [item["exit_center"] for item in history], label="Exit", color="#b45309")
    axes[1, 1].plot([item["iteration"] for item in history], [item["transition_width"] for item in history], label="Width", color="#7c3aed")
    axes[1, 1].set_title("Recovered parameters")
    axes[1, 1].set_xlabel("Iteration")
    axes[1, 1].legend(frameon=False, ncol=2)

    png_path = out_dir / "autodiff_fringing_response.png"
    pdf_path = out_dir / "autodiff_fringing_response.pdf"
    ...
    (out_dir / "autodiff_fringing_response_summary.json").write_text(json.dumps(summary, indent=2))
    return summary


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the LMX fringing multi-observable autodiff inverse-design demo.")
    parser.add_argument("--output", type=Path, default=Path("artifacts/examples/autodiff_fringing_response"))
    parser.add_argument("--steps", type=int, default=16)
    args = parser.parse_args(argv)
    run_autodiff_fringing_response_demo(out_dir=args.output, steps=args.steps)
    return 0
